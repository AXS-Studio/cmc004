	...
	$.widget("vanderlee.colorpicker", {
		...
		open: function() {
			...
			if (!that.opened) {
				...
				var center_hor = ( $(window).width()/2 ) - ( that.dialog.outerWidth() /2) ; //right/4;
				
				var center_vert = bottom/3;
				
				//that.dialog.css({'left': x, 'top': y});

				that.dialog.css({'left': center_hor, 'top': center_vert, 'position': 'absolute'});
